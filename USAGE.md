...
- **Купон и монетизация** — редактируемая таблица ставок (можно добавлять/
  удалять строки), кнопка «Сгенерировать купоны» → таблица комбинаций с
  положительным EV → выбор купона, ставка, комиссия платформы → выплата.
- **Страны/лиги** — выбор вида спорта и лиги из seed-реестра → список
  участников (демонстрация универсальной схемы сущностей).
- **О проекте** — границы объёма (что не реализовано и почему).
...
клубов мира (см. раздел 9, как подключить реальные данные). Баскетбол и
...
```bash
pytest tests/ -q     # 104 теста, без реальных сетевых/AI вызовов (используются фейки)
